...
def run_nsga2_scheduler(
    ...
    if not np.any(is_feasible_solution):
        ...
        # Group slots by shift ID
        shift_groups = {}
        for slot_idx, row in flattened_slots.iterrows():
            shift_id = row.get('MS Ca thi', f"shift_{slot_idx}")
            if shift_id not in shift_groups:
                shift_groups[shift_id] = []
            shift_groups[shift_id].append(slot_idx)

        print(f"\n[DEBUG] Assignment verification BEFORE JSON packaging:")
        print(f"  Total slots: {len(flattened_slots)}")
        print(f"  Total staff: {len(staff_df)}")
        print(f"  Total assignments in chromosome: {len(best_chromosome)}")
        print(f"  Unique shifts (MS Ca thi): {len(shift_groups)}")
        
        # Verify each shift has correct number of staff
        invalid_shifts = 0
        sample_shifts_checked = 0
        for shift_id, slot_indices in shift_groups.items():
            assigned_staff = [best_chromosome[idx] for idx in slot_indices]
            unique_staff = set(assigned_staff)
            required = len(slot_indices)
            actual = len(assigned_staff)
            has_dup = len(unique_staff) < len(assigned_staff)
            
            if sample_shifts_checked < 10:  # Show first 10
                status = "❌ DUP" if has_dup else "✓"
                print(f"    {status} Shift '{shift_id}': {actual} slots → {len(unique_staff)} unique staff (required {required})")
                sample_shifts_checked += 1
            
            if has_dup:
                invalid_shifts += 1
        
        if invalid_shifts > 0:
            print(f"  ⚠️  Found {invalid_shifts} shifts with duplicate staff in total!")
        else:
            print(f"  ✓ No duplicates - all shifts have correct unique staff")

        return best_chromosome, flattened_slots
    ...
    # ⚠️ POST-VALIDATION: Remove duplicates where same staff assigned to multiple slots of same shift
    best_chromosome = _remove_duplicate_staff_in_same_shift(best_chromosome, slots_df, problem)
...
# ═══════════════════════════════════════════════════════════════════
def _remove_duplicate_staff_in_same_shift(
    chromosome: np.ndarray,
    slots_df: pd.DataFrame,
    problem: 'ExamSchedulingProblem'
) -> np.ndarray:
    """
    Remove duplicates where the same staff is assigned to multiple slots of the same shift.
    
    When a shift requires N staff, it creates N slots in slots_df. The algorithm might
    assign the same staff to multiple slots of the same shift, which is invalid.
    
    This function detects and fixes such duplicates by reassigning duplicated slots
    to other available staff members.
    """
    print(f"\n[DEDUP] Kiểm tra và xóa những ca trùng (cùng ca thi gán 2 người)...")
    
    # Group slots by shift ID to identify which slots belong to the same shift
    shift_groups = {}
    for slot_idx, row in slots_df.iterrows():
        shift_id = row.get('MS Ca thi', f"shift_{slot_idx}")
        if shift_id not in shift_groups:
            shift_groups[shift_id] = []
        shift_groups[shift_id].append(int(slot_idx))
    
    duplicates_found = 0
    duplicates_fixed = 0
    
    num_staff = len(np.unique(chromosome))
    
    # For each shift, check if the same staff is assigned to multiple slots
    for shift_id, slot_indices in shift_groups.items():
        if len(slot_indices) <= 1:
            continue
        
        assigned_staff = [chromosome[idx] for idx in slot_indices]
        unique_staff = set(assigned_staff)
        
        # If number of assigned staff < number of slots, there's a duplicate
        if len(unique_staff) < len(slot_indices):
            duplicates_found += len(slot_indices) - len(unique_staff)
            print(f"  ⚠️  Ca thi '{shift_id}': Có {len(assigned_staff)} slot nhưng chỉ {len(unique_staff)} cán bộ riêng")
            
            # Try to reassign duplicated slots to different staff
            for i, slot_idx in enumerate(slot_indices):
                staff_idx = chromosome[slot_idx]
                
                # Count how many times this staff is assigned in this shift
                count_in_shift = sum(1 for j in slot_indices if chromosome[j] == staff_idx)
                
                if count_in_shift > 1:
                    # Find a replacement staff who isn't already in this shift
                    for replacement_staff in range(num_staff):
                        if replacement_staff not in assigned_staff:
                            # Check if this reassignment violates hard constraints
                            if slot_idx in getattr(problem, 'conflict_map', {}):
                                conflicting = any(chromosome[c_slot] == replacement_staff
                                                for c_slot in problem.conflict_map[slot_idx])
                                if conflicting:
                                    continue
                            
                            # Perform the reassignment
                            old_staff = chromosome[slot_idx]
                            chromosome[slot_idx] = replacement_staff
                            duplicates_fixed += 1
                            print(f"    ✓ Thay thế slot {slot_idx}: cán bộ {old_staff} → {replacement_staff}")
                            assigned_staff[i] = replacement_staff
                            break
    
    if duplicates_found > 0:
        print(f"[DEDUP] Tổng cộng: Tìm {duplicates_found} ca trùng, sửa {duplicates_fixed} ca")
    else:
        print(f"[DEDUP] Không tìm ca trùng nào ✓")
